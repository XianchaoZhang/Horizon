...
/* Excerpt from the paper: "Following interpolation of all missing G values, B/R values are interpolated at R/B pixels using the full green color pane."*/
    ...
    Func fin("fin");
    Func b_ne, b_se, b_sw, b_nw, w_ne, w_se, w_sw, w_nw, b_ne_est, b_se_est, b_sw_est, b_nw_est;
    ...
    Func g_n, g_e, g_s, g_w, w_n, w_e, w_s, w_w, g_n_est, g_e_est, g_s_est, g_w_est, input_16;
...
*/
    ...
    fin(r_p.x, r_p.y, 0) = ((w_ne(r_p.x, r_p.y) * b_ne_est(r_p.x, r_p.y) + w_se(r_p.x, r_p.y) * b_se_est(r_p.x, r_p.y) + w_sw(r_p.x, r_p.y) * b_sw_est(r_p.x, r_p.y) + w_nw(r_p.x, r_p.y) * b_nw_est(r_p.x, r_p.y)) / (w_ne(r_p.x, r_p.y) + w_se(r_p.x, r_p.y) + w_sw(r_p.x, r_p.y) + w_nw(r_p.x, r_p.y)));

    fin(b_p.x, b_p.y, 2) = ((w_ne(b_p.x, b_p.y) * b_ne_est(b_p.x, b_p.y) + w_se(b_p.x, b_p.y) * b_se_est(b_p.x, b_p.y) + w_sw(b_p.x, b_p.y) * b_sw_est(b_p.x, b_p.y) + w_nw(b_p.x, b_p.y) * b_nw_est(b_p.x, b_p.y)) / (w_ne(b_p.x, b_p.y) + w_se(b_p.x, b_p.y) + w_sw(b_p.x, b_p.y) + w_nw(b_p.x, b_p.y)));
